...
fill_oid(daos_unit_oid_t oid, struct dss_enum_arg *arg)
{
	...
	type = vos_iter_type_2pack_type(VOS_ITER_OBJ);
	...
	D_ASSERT(iovs[arg->sgl_idx].iov_len + sizeof(oid) <
		 iovs[arg->sgl_idx].iov_buf_len);
	memcpy(iovs[arg->sgl_idx].iov_buf + iovs[arg->sgl_idx].iov_len,
	       &oid, sizeof(oid));
	iovs[arg->sgl_idx].iov_len += sizeof(oid);

	D_DEBUG(DB_IO, "Pack obj "DF_UOID" iov_len %zu kds_len %d\n",
		DP_UOID(oid), iovs[arg->sgl_idx].iov_len, arg->kds_len);
	return 0;
}
